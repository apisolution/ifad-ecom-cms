    ...
    <div class="row">
        ...
        <div class="col-xl-12">
            ...
            <div class="dt-entry__header">
                ...
                <!-- /entry heading -->
                @if (permission('document-add'))
                <button class="btn btn-primary btn-sm" onclick="showStoreFormModal('Add New document','Save')">
...
$(document).ready(function(){
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('document.datatable.data')}}",
            "type": "POST",
            "data": function (data) {
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        let url = "{{route('document.store.or.update')}}";
        ...
        if (id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#store_or_update_form #document_count').val(data.document_count);
                    $('#store_or_update_form #document_order').val(data.document_order);
                    $('#store_or_update_form #document_desc').val(data.document_desc);

                    if(data.document_file){
                        $('#store_or_update_form .fileName').val(data.document_file);
                    }

                    //$('#store_or_update_form #document_category_id').val(data.document_category_id);


                    let riMenuArray = data.document_category_id.split(',');
                    console.log(riMenuArray);
                    let riMenuInputs = $('.riMenuInputs input[type="radio"]');
                    ...
                    if(data.image){
                        var image = "{{ asset('storage/'.DOCUMENT_IMAGE_PATH)}}/"+data.image;
                    ...
                    //$('#store_or_update_form #document_file').val(data.document_file);
                   /*  $('#store_or_update_form .selectpicker').selectpicker('refresh');
                    if(data.document_file){
                        var document_file = "{{ asset('storage/'.DOCUMENT_IMAGE_PATH)}}/"+data.document_file;
                        $('#store_or_update_form #document_file').attr('src',document_file);
                    }else{
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{route('document.bulk.delete')}}";
    ...
    $(document).on('click', '.change_status', function () {
        ...
        let name  = $(this).data('name');
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('document.change.status') }}";
